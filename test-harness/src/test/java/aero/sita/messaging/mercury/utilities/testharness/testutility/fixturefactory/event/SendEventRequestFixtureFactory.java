/*
 * This code contains copyright information which is the proprietary property
 * of SITA Information Network Computing Limited (SITA). No part of this
 * code may be reproduced, stored or transmitted in any form without the prior
 * written permission of SITA.
 * Copyright © SITA Information Networking Computing Ireland Limited 2020-2025.
 * Confidential. All rights reserved.
 */

package aero.sita.messaging.mercury.utilities.testharness.testutility.fixturefactory.event;

import static aero.sita.messaging.mercury.utilities.testharness.testutility.fixturefactory.event.IncomingEventDtoFixtureFactory.defaultIncomingEventDto;

import aero.sita.messaging.mercury.utilities.testharness.api.v1.event.request.SendEventRequest;
import java.util.List;

public class SendEventRequestFixtureFactory {

  private SendEventRequestFixtureFactory() {
    throw new IllegalStateException("Utility class");
  }

  public static SendEventRequest defaultSendEventRequest() {
    return SendEventRequest.builder()
        .incomingEvent(defaultIncomingEventDto())
        .topics(List.of("evh-foo"))
        .build();
  }
}
